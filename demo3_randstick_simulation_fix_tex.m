...
f = 0.5;        % targeted intra-cellular volume fraction
...
for i = 1:numel(cv)
    ...
    for j = 1:numel(f)
        ...
        RMS = rmsobj();
        filename = sprintf('cv%02u', cvi*100);
...
for i = 1:numel(cv)
    ...
    for j = 1:numel(f)
        ...
        perm = (1-fi)/(tex*S/V);        % permeability, um/ms
...
t   = sim.TD;
save(fullfile(root,'simulation_results_fix_tex.mat'),'cv','f','fgt','t','MD','MK','tex');
