classdef randstick < handle
    ...
    methods (Access = public)
        function this = randstick(varargin)
            
        end
        
        function BW = multistick(this,Nvox,Lvox,pos,dir,seed,abar,astd,lbar,rcsa,cv)
            [~, Y2, L] = this.stickend(pos, dir);
            [Y, X, Z] = meshgrid(1:Nvox, 1:Nvox, 1:Nvox);
            res = Lvox/Nvox;
            BW = false(Nvox,Nvox,Nvox);
            for ii = 1:size(pos,1)
                seed = seed + 1;
                N = ceil(Nvox*L(ii));
                pb = this.randbeadpos(seed,N,L(ii)*Lvox,abar,astd);
                rb = this.randbeadrad(N,L(ii)*Lvox,pb,lbar,rcsa,cv);
                rb = rb/res;
                t = linspace(0,1,N);
                for i = 1:N
                    xi = Y2(ii,:) + dir(ii,:)*L(ii)*t(i);
                    xi = xi*Nvox;
%                     BWi = (X-xi(1)).^2 + (Y-xi(2)).^2 + (Z-xi(3)).^2 <= rb(i).^2;
%                     BW(BWi) = true;
                    Xrange = max(1,floor(xi(1)-rb(i))) : min(Nvox,ceil(xi(1)+rb(i)));
                    Yrange = max(1,floor(xi(2)-rb(i))) : min(Nvox,ceil(xi(2)+rb(i)));
                    Zrange = max(1,floor(xi(3)-rb(i))) : min(Nvox,ceil(xi(3)+rb(i)));
                    Xi = X(Xrange,Yrange,Zrange);
                    Yi = Y(Xrange,Yrange,Zrange);
                    Zi = Z(Xrange,Yrange,Zrange);
                    BWi = (Xi-xi(1)).^2 + (Yi-xi(2)).^2 + (Zi-xi(3)).^2 <= rb(i).^2;
                    BW(Xrange,Yrange,Zrange) = logical(BW(Xrange,Yrange,Zrange)+BWi);
                end
            end
        end
        
        function BW = onestick(this,Nvox,Lvox,pos,dir,seed,abar,astd,lbar,rcsa,cv)
            [~, Y2, L] = this.stickend(pos, dir);
            [Y, X, Z] = meshgrid(1:Nvox, 1:Nvox, 1:Nvox);
            N = ceil(Nvox*L);
            pb = this.randbeadpos(seed,N,L*Lvox,abar,astd);
            rb = this.randbeadrad(N,L*Lvox,pb,lbar,rcsa,cv);
            res = Lvox/Nvox;
            rb = rb/res;
            t = linspace(0,1,N);
            BW = false(Nvox,Nvox,Nvox);
        ...
        function [Y1, Y2, L] = stickend(this, X, n)
            % X: position, N x 3 matrix
            % n: direction, N x 3 matrix
            N = size(X,1);
            t = [(1-X)./n, -X./n];
            t1 = zeros(N,1);
            for i = 1:N
            ...
            L = sqrt(sum((Y1-Y2).^2,2));
        end
        
        function this = plotstick(this, Y1, Y2)
            N  = size(Y1,1);
            hold on;
            for i = 1:N
                plot3([Y1(i,1), Y2(i,1)],[Y1(i,2), Y2(i,2)],[Y1(i,3), Y2(i,3)],'o-');
            end
            view(3)
            pbaspect([1 1 1]);
            box on; grid on;
