...
for i = 1:numel(cv)
    ...
    for j = 1:numel(tex)
        ...
        RMS = rmsobj();
        filename = sprintf('cv%02u_tex%03u', cvi*100, texj);
...
f = 0.5;              % intra-cellular volume fraction
i = 1; j = 1;
proj = sprintf('cv%02u_tex%03u', cv(i)*100, tex(j));
...
for i = 1:numel(cv)
    ...
    for j = 1:numel(tex)
        ...
        texj = tex(j);
        % Project name
        proj = sprintf('cv%02u_tex%03u', cvi*100, texj);
        ...
        perm = (1-fi)/(texj*S/V);       % permeability, um/ms
...
t   = sim.TD;
save(fullfile(root,'simulation_results_fix_two_tex.mat'),'cv','f','fgt','t','MD','MK','tex');
